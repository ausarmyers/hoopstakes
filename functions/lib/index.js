...
exports.getLeoBreakdown = exports.getAdminKpis = exports.ingestAnalyticsEvent = exports.openMatchDispute = exports.confirmMatchResult = exports.submitMatchResult = exports.requestCashout = exports.stripeWebhook = exports.createStripeCheckout = void 0;
...
function cashoutMinimumForTier(tier) {
    if (tier === 'hoopster')
        return 10;
    if (tier === 'elite')
        return 5;
...
function hooperTierFromLeo(score) {
    if (score >= 90)
        return 'Legend Circuit';
    if (score >= 80)
        return 'Elite Run';
    if (score >= 70)
        return 'Gym Proven';
    if (score >= 60)
        return 'Street Certified';
...
exports.stripeWebhook = functions.https.onRequest(async (req, res) => {
    ...
    try {
        if (event.type === 'checkout.session.completed') {
            ...
            const tier = (session.metadata?.tier || 'rookie');
...
exports.requestCashout = functions.https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    ...
    await db.runTransaction(async (tx) => {
        ...
        const tier = String(user?.tier || 'rookie').toLowerCase();
        ...
        if (amount < min) {
            throw new functions.https.HttpsError('failed-precondition', `Minimum cashout for ${tier} is $${min.toFixed(2)}`);
        }
        ...
        if (postCashout < 1) {
            throw new functions.https.HttpsError('failed-precondition', 'You must retain at least $1 after cashout');
        }
        tx.set(userRef, {
...
exports.confirmMatchResult = functions.https.onCall(async (data, context) => {
    const uid = requireAuth(context);
...
exports.openMatchDispute = functions.https.onCall(async (data, context) => {
    const uid = requireAuth(context);
    const matchId = String(data?.matchId || '');
...
exports.ingestAnalyticsEvent = functions.https.onCall(async (data, context) => {
...
exports.getAdminKpis = functions.https.onCall(async (_data, context) => {
    const uid = requireAuth(context);
    ...
    const count = (name) => events.filter((e) => e.eventName === name).length;
...
exports.getLeoBreakdown = functions.https.onCall(async (data, context) => {
    const uid = requireAuth(context);
